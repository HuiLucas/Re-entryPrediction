# Import time and begin assessing runtime
import time
starttime = time.time()
...
# Imports for fancy parallelization
import multiprocessing as mp


# Delfi datasets: [name, norad_cat_id, mass, reference area, drag coefficient, radiation pressure coefficient, launch date]
C3_data =   ["Delfi-C3",   32789, 2.2, 0.080, 1.7, 1.1,  "2008-04-28"]  # True C3 re-entry: 2023-11-13 (433 days after 2022-09-06)
...
propagation_duration = 9999                         # How long to propagate for at most [days]
...
# Function to get the tle from spacetrack
...
if __name__ == "__main__":
    # CD values to test
    CD_values = np.random.normal(CD_mean, CD_sd, N)
    inputs = CD_values.tolist()
    n_cores = mp.cpu_count()
    with mp.get_context("spawn").Pool(n_cores) as pool:
        outputs = pool.map(EOL, inputs)

    # Plotterdeplot
    now = datetime.now()
    plt.figure(figsize=(9, 5))
    plt.title(f"EOL distribution of {satellite} - {now.day}-{now.month}-{now.year} {now.hour}h{now.minute}")
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"PREDICTIONS_RANGE\{satellite}\EOL distribution of {satellite} - {now.day}-{now.month}-{now.year} {now.hour}h{now.minute}")


    # Create text file with all inputs and outputs
    atmomodel = "NRLMSISE-00"
    integrator_used = "Runge-Kutta 78, fixed time step"
    endtime = time.time()
    runtime = endtime - starttime
    print(f"Ran in {runtime} seconds or {runtime/60} minutes")

    ff = open(f"PREDICTIONS_RANGE\{satellite}\{satellite} EOL Prediction - {now.day}-{now.month}-{now.year} {now.hour}h{now.minute}", "w")
    ff.write(f"============== End-of-Life Prediction for {satellite} ==============\n")
    ff.write(f"### File creation on {datetime.now()} \n")
    ff.write("\n")
    ff.write("============================== PC ============================== \n")
    ff.write(f"Cores used: {n_cores}")
    ff.write("\n")
    ff.write("============================ INPUTS ============================ \n")
    ...
    ff.write(f"Drag coefficient mean:               {CD_mean} \n")
    ff.write(f"Drag coefficient standard deviation: {CD_sd} \n")
    ff.write(f"Number of samples:                   {N} \n")
    ...
    ff.write(f"Associated graph: EOL distribution of {satellite} - {now.day}-{now.month}-{now.year} {now.hour}h{now.minute}")
