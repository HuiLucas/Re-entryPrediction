...
#Calculates the Drag Coefficient step size
step_size  = (drag_coefficient_upper - drag_coefficient_lower)/(iterations-1)
#Calculates the epochs for the comparisons
Comp_Epochs = [TLE1_number + (i+1)*int((TLE2_number-TLE1_number)/Comparisons) for i in range(Comparisons)] 
...
for i in range(time_frames):
    ...
    print('\n')
    print('---------------------------------')
    print('STARTING TIME FRAME', i+1)
    print('---------------------------------')

    Error = []
    DC = []

    current_TLE1 = TLE1_number + i*int((TLE2_number-TLE1_number)/time_frames)
    current_TLE2 = TLE1_number + (i+1)*int((TLE2_number-TLE1_number)/time_frames)
    current_Comp_Epochs = Comp_Epochs[int(len(Comp_Epochs)/time_frames*i):int(len(Comp_Epochs)/time_frames*(i+1))]

    TLE_1 = split_TLE_sets[current_TLE1]
    TLE_2 = split_TLE_sets[current_TLE2]

    parsed_TLE1 = parse_TLE(TLE_1)
    parsed_TLE2 = parse_TLE(TLE_2)

    parsed_TLE_LST = [parsed_TLE1] #List of parsed TLEs starting from TLE1 to TLE2 with the step size of Comparisons
    for j in current_Comp_Epochs:
        parsed_TLE_LST.append(parse_TLE(split_TLE_sets[j]))
    ...
    sim_epochs=[]
    for j in range(int(Comparisons/time_frames)):
        Comp_date = DateTime(parsed_TLE_LST[j+1]['Year_Actual'], parsed_TLE_LST[j+1]['Month'], parsed_TLE_LST[j+1]['Day'], parsed_TLE_LST[j+1]['Hour']).epoch()
    ...
    # Gets starting state
    ephemerisdef = environment.TleEphemeris( "Earth", "J2000", initTLE[current_TLE1], False )
    state = ephemerisdef.cartesian_state(simulation_start_epoch)


    # Creates list of states at each TLE
    ephemerisdef_lst = []
    state_lst = []
    for j in range(int(Comparisons/time_frames)):
        TLE_number = int(current_Comp_Epochs[j])
        ...
        state_Comp = ephemersisdef_Comp.cartesian_state(sim_epochs[j]) 
    ...
    ACT_Semi_major_lst = []
    for j in range(int(Comparisons/time_frames)):
        Semi_major_axis = element_conversion.cartesian_to_keplerian(state_lst[j], bodies.get("Earth").gravitational_parameter)[0]
        ACT_Semi_major_lst.append(Semi_major_axis)
    ...
    dir_name = "Error vs Cd 2/" + str(str(TLE1_number) + " to " + str(TLE2_number) + 
    ...
    for k in range(iterations):
        ...
        # Set the drag coefficient for this iteration
        drag_coefficient = drag_coefficient_lower + k * step_size
        DC.append(drag_coefficient)

        print("\n")
        print("starting the propagation for a drag coefficient of", drag_coefficient)


    # Create aerodynamic and acceleration settings
    ...
    #This makes a list of the predicted semi-major axis at the epochs that are compared to the TLE data
        ...
        for j in range(len(indices_in_Output_Epochs)):
            SM_Comp_lst.append(SMA[indices_in_Output_Epochs[j]])


    # Calculate the Mean Squared Error
        MSE = 0
        for j in range(len(SM_Comp_lst)):
            MSE = MSE + ((ACT_Semi_major_lst[j] - SM_Comp_lst[j])**2)
    ...
    # Plot the predicted semi-major axis vs the actual semi-major axis
        ...
        plt.grid(True)
        plt.savefig(f'{dir_name}/Real vs Predicted SMA {drag_coefficient}, {i+1}.png')
...
for i in range(time_frames):
    plt.plot(DC_lst[i], Error_lst[i], label = "Time Frame " + str(i+1))
#plt.plot(DC, Error)
plt.xlabel('DC')
plt.ylabel('Error')
plt.grid()
